...
public partial class FoodFixture
{
    ...
    public async Task CanUpdateFood()
    {
        ...
        {
            var beforeUpdateFood = await repository.GetByIdAsync<Foods>(11).ConfigureAwait(false);
    ...
    public async Task CanDeleteFood()
    {
        ...
        await Run<IRepository>(async respository =>
        {
            var beforeDeleteFood = await respository.GetByIdAsync<Foods>(food.Id);
